...
METRIC_TYPES = {'c': 'COUNTER', 'g': 'GAUGE', 'ms': 'TIMER',
                's': 'SET', 'h': 'HISTOGRAM', 'm': 'METER'}
...
def regex_parse_message(message):
    ...
    try:
        for msg in re.finditer(REGEX, message):
            if msg.group(0):
                ...
                if msg.group('metric_name'):
                    ...
                    mtype = METRIC_TYPES[msg.group('metric_type')]
        ...
        ret['counts'] = {'messages': message_count, 'events': event_count}
...
class Poster(threading.Thread):
    ...
    def flush(self):
        try:
            ...
            self.element.add_sample(
                self.stats_prefix + '.packets_received.count', int(time.time()), self.packet_count, 'GAUGE', host=self.hostname)
            self.element.add_sample(
                self.stats_prefix + '.samples_received.count', int(time.time()), self.message_count, 'GAUGE', host=self.hostname)

            self.element.add_sample(
                self.stats_prefix + '.event_received.count', int(time.time()), self.event_count, 'GAUGE', host=self.hostname)

            self.element.add_sample(
                self.stats_prefix + '.packets_received.rate.' + str(self.interval), int(time.time()), self.get_rate(self.packet_count, self.interval), 'COUNTER', host=self.hostname)
            self.element.add_sample(
                self.stats_prefix + '.samples_received.rate.' + str(self.interval), int(time.time()), self.get_rate(self.message_count, self.interval), 'COUNTER', host=self.hostname)
            self.element.add_sample(
                self.stats_prefix + '.event_received.rate.' + str(self.interval), int(time.time()), self.get_rate(self.event_count, self.interval), 'COUNTER', host=self.hostname)

            LOG.debug('Message count: {0}'.format(self.message_count))
            LOG.debug('Packet count: {0}'.format(self.packet_count))
            LOG.debug('Event count: {0}'.format(self.event_count))

            sl = len(self.element.samples)

            LOG.debug('Flushing {0} samples'.format(sl))

            # send the data to Netuitive
            self.api.post(self.element)

            LOG.info('Successfully sent {0} samples'.format(sl))
    ...
    def submit(self, message, timestamp):
        ...
        if messages is not None:
            self.message_count += messages['counts']['messages']
            self.event_count += messages['counts']['events']
...
if __name__ == "__main__":
    ...
    try:
        ...
        Element = netuitive.Element(ElementType=CONFIG['element_type'])
        add_metadata(Element, tags)

        # setup Netuitive API posting thread
        poster = Poster(CONFIG, Element)
